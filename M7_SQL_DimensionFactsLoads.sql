...
Module 6: Creating Dimensional Tables
Date: 7/27/2022
...
--Load STAGE_PRODUCT into Dim_Product
INSERT INTO Dim_Product
(DimProductID 
	,ProductID 
    ,ProductTypeID
...
FROM STAGE_PRODUCT AS P
    INNER JOIN STAGE_PRODUCTTYPE PT ON P.ProductTypeID = PT.ProductTypeID
    INNER JOIN STAGE_PRODUCTCATEGORY PC ON PT.ProductCategoryID = PC.ProductCategoryID
...
---Load unknowns into Dim_Location
...
---Load from STAGE_CUSTOMER into Dim_Location
...
---Load from STAGE_RESELLER into Dim_Location    
    SELECT Address, City, PostalCode, StateProvince, Country
...
---Load from STAGE_STORE into Dim_Location
    SELECT Address, City, PostalCode, StateProvince, Country
...
---Load from STAGE_STORE into Dim_Store
INSERT INTO Dim_Store
(DimStoreID, SourceStoreID, StoreNumber, StoreManager)
    SELECT StoreID, StoreID, StoreNumber, StoreManager

    FROM STAGE_STORE;

---CREATE Dim_Reseller
CREATE OR REPLACE TABLE Dim_Reseller(
    DimResellerID VARCHAR(255) CONSTRAINT PK_DimResellerID PRIMARY KEY NOT NULL --Surrogate Key
    ,DimLocationID INTEGER CONSTRAINT FK_DimLocationIDReseller FOREIGN KEY REFERENCES Dim_Location (DimLocationID) 
    ,ResellerID VARCHAR(255) --Natural key
...
---Load from STAGE_RESELLER into Dim_Reseller
INSERT INTO Dim_Reseller
(DimResellerID, ResellerID, ResellerName, ContactName, PhoneNumber, Email)
    SELECT ResellerID, ResellerID, ResellerName, Contact, PhoneNumber, EmailAddress

    FROM STAGE_RESELLER;

---Create Dim_Customer
CREATE OR REPLACE TABLE Dim_Customer(
    DimCustomerID VARCHAR(255) CONSTRAINT PK_DimCustomerID PRIMARY KEY NOT NULL --Surrogate Key
    ,DimLocationID INTEGER CONSTRAINT FK_DimLocationIDCustomer FOREIGN KEY REFERENCES Dim_Location (DimLocationID) 
    ,CustomerID VARCHAR(255) --Natural key
...
---Load from STAGE_CUSTOMER into Dim_Customer
...
(    DimCustomerID 
    ...
    ,CustomerLastName 
    ,CustomerGender)
    SELECT CustomerID, CustomerID, CONCAT(FirstName, ' ', LastName), FirstName, LastName, Gender
    
    FROM STAGE_CUSTOMER

---CREATE Dim_Channel
CREATE OR REPLACE TABLE Dim_Channel(
    DimChannelID INT CONSTRAINT PK_DimChannelID PRIMARY KEY NOT NULL --Surrogate Key
...
---Load from stage tables to Dim_Channel
INSERT INTO Dim_Channel
    (DimChannelID 
    ,ChannelID 
    ,ChannelName
    ,ChannelCategoryID 
     ,ChannelCategory)
    SELECT C.ChannelID, C.ChannelID, C.Channel, CC.ChannelCategoryID, CC.ChannelCategory
    FROM STAGE_CHANNEL AS C
    INNER JOIN STAGE_CHANNELCATEGORY CC ON C.ChannelCategoryID = CC.ChannelCategoryID
...
( DimStoreID INTEGER CONSTRAINT FK_DimStoreID FOREIGN KEY REFERENCES Dim_Store(DimStoreID) -- Foreign key
 ...
 ,DimTargetTypeDateID NUMBER(9) CONSTRAINT FK_DimTargetTypeDateID FOREIGN KEY REFERENCES Dim_Date(DATE_PKEY) -- Foreign key
...
INSERT INTO Fact_SRCSalesTarget
 ...
 ,SalesTargetAmount
    ...
    ,'Dim_Date.DimDateID'
    ,('STAGE_TARGET_CHANNELRESELLERSTORE.TargetSalesAmount'/365)
    FROM STAGE_TARGET_CHANNELRESELLERSTORE AS CRS
    INNER JOIN Dim_Channel C ON CRS.ChannelName = C.ChannelName
    INNER JOIN STAGE_SALESHEADER SH ON C.DimChannelID = SH.ChannelID 
    INNER JOIN Dim_Store S ON SH.StoreID = S.DimStoreID
    INNER JOIN Dim_Reseller R ON SH.ResellerID = R.DimResellerID
    INNER JOIN Dim_Date D ON SH.Date = D.Date
...
(
  ...
  ,ProductTargetSalesQuantity 
    ...
    FROM STAGE_TARGET_PRODUCT AS TP
    INNER JOIN Dim_Product P ON TP.ProductID = P.DimProductID
    INNER JOIN Dim_Date D ON TP.YEAR = D.Date
...
(
  ...
  ,SaleTotalProfit
    ...
    SELECT
        ...
        ,'STAGE_SALESDETAIL.SalesAmount'
        ,'STAGE_SALESDETAIL.SalesQuantity'
        ,('STAGE_SALESDETAIL.SalesAmount'/'STAGE_SALESDETAIL.SalesQuantity')
        ,('Dim_Product.ProductCost' * 'STAGE_SALESDETAIL.SalesQuantity')
        ,('STAGE_SALESDETAIL.SalesAmount' - ('Dim_Product.Cost' * 'STAGE_SALESDETAIL.SalesQuantity'))
    FROM STAGE_SALESDETAIL AS SD
        INNER JOIN STAGE_SALESHEADER SH ON SD.SalesHeaderID = SH.SalesHeaderID
        INNER JOIN Dim_Product P ON SD.ProductID = P.DimProductID
        INNER JOIN Dim_Reseller R ON SH.ResellerID = R.DimResellerID
        INNER JOIN Dim_Customer CU ON SH.CustomerID = CU.DimCustomerID
        INNER JOIN Dim_Channel C ON SH.ChannelID = C.DimChannelID
        INNER JOIN Dim_Date D ON SH.Date = D.Date
        INNER JOIN Dim_Location L ON R.DimLocationID = L.DimLocationID
        INNER JOIN Dim_Store S ON L.DimLocationID = S.DimLocationID